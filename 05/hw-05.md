...
```
- Далее перешел в проект vpc. Там реализовал ```terraform_remote_state```. И реализовал ```backend "s3"```. Далее необходимо запустить команду для миграции ипередать в нее полученнные ранее  ```bucket_name, access_key``` и ```secret_key```:
...
secret_key = data.terraform_remote_state.storage.outputs.secret_key
```
а значит они попадут в state файлю.
